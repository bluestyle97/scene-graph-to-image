...
#
# Modified from https://github.com/google/sg2im/blob/master/sg2im/losses.py
...
def gradient_penalty(x_real, x_fake, f, gamma=1.0):
    N = x_real.size(0)
    eps = torch.randn(N, 1, 1, 1, device=x_real.device, dtype=x_real.dtype)
...
def gan_g_loss(scores_fake, gan_loss_type='gan'):
    assert gan_loss_type in ['gan', 'wgan', 'lsgan']
...
def gan_d_loss(scores_real, scores_fake, gan_loss_type='gan'):
